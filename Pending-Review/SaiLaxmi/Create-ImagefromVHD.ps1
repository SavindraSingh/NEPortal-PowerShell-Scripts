...
Begin
{
    ...
    Function Validate-AllParameters
    {
        Try
        {
            ...
            # Validate parameter: VHDUri
            Write-LogFile -FilePath $LogFilePath -LogText "Validating Parameters: VHDUri. Only ERRORs will be logged."
            If([String]::IsNullOrEmpty($VHDUri))
            {
                Write-LogFile -FilePath $LogFilePath -LogText "Validation failed. VHDUri parameter value is empty.`r`n<#BlobFileReadyForUpload#>"
                $ObjOut = "Validation failed. VHDUri parameter value is empty."
                $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                Write-Output $output
                Exit
            }
            ...
            If([String]::IsNullOrEmpty($OSType))
            {
                ...
                $ObjOut = "Validation failed. OSType parameter value is empty."
                $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                Write-Output $output
                Exit
            }

            # Validate parameter: OSDiskName
            Write-LogFile -FilePath $LogFilePath -LogText "Validating Parameters: OSDiskName. Only ERRORs will be logged."
            If([String]::IsNullOrEmpty($OSType))
            {
                Write-LogFile -FilePath $LogFilePath -LogText "Validation failed. OSDiskName parameter value is empty.`r`n<#BlobFileReadyForUpload#>"
                $ObjOut = "Validation failed. OSDiskName parameter value is empty."
                $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                Write-Output $output
                Exit
            }
...
Process
{
    ...
    Try{
        Write-LogFile -FilePath $LogFilePath -LogText "Trying create Image from VHD"
        $imagevhd = (Add-AzureDisk -DiskName $OSDiskName -MediaLocation $VHDUri -OS $OSType -ErrorAction Stop) | Out-Null
        Write-LogFile -FilePath $LogFilePath -LogText "Successfully created Image from VHD"
        $ObjOut = "Successfully created Image from VHD."
    ...
    Catch
    {
        Write-LogFile -FilePath $LogFilePath -LogText "Error While Creating Image from VHD.`r`n<#BlobFileReadyForUpload#>"
        $ObjOut = "Error While Creating Image from VHD."
        $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
        Write-Output $output
        Exit
    }
