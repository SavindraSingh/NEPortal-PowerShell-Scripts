<#
    ...
    .NOTES
     Purpose of script:     The script is to create virtual machine from custom image.
...
Begin
{
    ...
    Function Validate-AllParameters
    {
        Try
        {
            ...
            # Validate parameter: OSImageURI
            Write-LogFile -FilePath $LogFilePath -LogText "Validating Parameters: OSImageURI. Only ERRORs will be logged."
            If([String]::IsNullOrEmpty($OSImageURI))
            {
                Write-LogFile -FilePath $LogFilePath -LogText "Validation failed. OSImageURI parameter value is empty.`r`n<#BlobFileReadyForUpload#>"
                $ObjOut = "Validation failed. Location OSImageURI value is empty."
                $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                Write-Output $output
                Exit
            }
