        ...
        if($RestoreExtensionStatus.StatusCode -eq 'OK')
        {
            ...
            if($InstallationStatus -ne $null)
            {
                ...
                if(($ExtScriptStatus.Statuses.Code -eq 'ProvisioningState/succeeded'))
                {
                    ...
                    Else 
                    {
                        Write-LogFile -FilePath $LogFilePath -LogText "Database has not been restored successfully on $VMName.$message`r`n<#BlobFileReadyForUpload#>"
                        $ObjOut = "Database has not been restored successfully on $VMName.$message"
                        $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                        Write-Output $output
                        Exit                        
                    }
