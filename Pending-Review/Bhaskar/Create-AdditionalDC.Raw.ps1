<##
    .SYNOPSIS
    The script is to create additional dc in Azure.
    ...
    .EXAMPLE
    C:\PS> .\Create-AdditionalDC.ps1 -ClientID 12345 -AzureUserName bhaskar@desharajubhaskaroutlook.onmicrosoft.com -AzurePassword Pa55w0rd1! -AzureSubscriptionID 13483623-4785-4789-8d13-b58c06d37cb9 -VMName testImage123 -DomainName mylab.local -DomainAdmin 'mylab.local\azure-admin' -PasswordForDomainAdmin ************* -ResourceGroupName MyResourceGrp -Location 'Southeast Asia'
...
Begin
{
    ...
    Function Validate-AllParameters
    {
        Try
        {
            ...
            # Validate parameter: DomainAdmin
            Write-LogFile -FilePath $LogFilePath -LogText "Validating Parameters: DomainAdmin. Only ERRORs will be logged."
            If([String]::IsNullOrEmpty($DomainAdmin))
            {
                Write-LogFile -FilePath $LogFilePath -LogText "Validation failed. DomainAdmin parameter value is empty.`r`n<#BlobFileReadyForUpload#>"
                $ObjOut = "Validation failed. DomainAdmin parameter value is empty."
                $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                Write-Output $output
                Exit
            }
...
Process
{
    ...
    # 4. Install the AD DS role and configure the addtional dc
    Try
    {
        ...
        $ExtensionName = "InstallAdditionalDC"
        Write-LogFile -FilePath $LogFilePath -LogText "Trying to Set the Extension for installing the additional dc."

        ($ADDCInstallationStatus = Set-AzureRmVMCustomScriptExtension -Name $ExtensionName -FileUri "https://automationtest.blob.core.windows.net/customscriptfiles/InstallAD-Windows2012.ps1" -Run Create-FirewallSettingsCS.ps1 -Argument "$DomainName $DomainAdmin $PasswordForDomainAdmin" -ResourceGroupName $ResourceGroupName -Location $Location -VMName $VMName -TypeHandlerVersion 1.8 -ErrorAction Stop -WarningAction SilentlyContinue) | Out-Null

        if($ADDCInstallationStatus.StatusCode -eq 'OK')
        {
            ...
            if($InstallationStatus -ne $null)
            {
                ...
                ($ScriptStatus = Get-AzureRMVM -Name $VMName -ResourceGroupName $ResourceGroupName -ErrorAction SilentlyContinue -WarningAction SilentlyContinue) | Out-Null
                ...
                if(($ExtScriptStatus.Statuses.Code -eq 'ProvisioningState/succeeded'))
                {
                    ...
                    if($message -eq $null)
                    {
                        ...
                        $ObjOut = "Additional DC installtion and Configuration has been done successfully for $VMName."
                        $output = (@{"Response" = [Array]$ObjOut; Status = "Success"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                        Write-Output $output
                    }
                    Else 
                    {
                        Write-LogFile -FilePath $LogFilePath -LogText "Additional DC and Configuration has not been installed successfully $VMName..$message`r`n<#BlobFileReadyForUpload#>"
                        $ObjOut = "Additional DC and Configuration has not been installed successfully $VMName..$message"
                        $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                        Write-Output $output
                        Exit                        
                    }
                ...
                {
                    Write-LogFile -FilePath $LogFilePath -LogText "Provisioning the script for AD installation was failed $VMName.`r`n<#BlobFileReadyForUpload#>"
                    $ObjOut = "Provisioning the script for AD installation was failed $VMName."
                    $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                    Write-Output $output
                    Exit
                }
            ...
            {
                Write-LogFile -FilePath $LogFilePath -LogText "The extesnion was not installed for AD Installtion on $VMName.`r`n<#BlobFileReadyForUpload#>"
                $ObjOut = "The extesnion was not installed for AD Installtion on $VMName."
                $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                Write-Output $output
                Exit
            }           
        ...
        Else
        {
            Write-LogFile -FilePath $LogFilePath -LogText "Unable to install the custom script extension For Additional AD Installtion for Virtual Machine.`r`n<#BlobFileReadyForUpload#>"
            $ObjOut = "Unable to install the custom script extension For Additional AD Installtion for Virtual Machine."
            $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
            Write-Output $output
            Exit            
        }
