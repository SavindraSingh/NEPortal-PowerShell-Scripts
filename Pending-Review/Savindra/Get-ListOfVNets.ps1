<#
    .SYNOPSIS
    Script to Get List of Virtual Networks

    .DESCRIPTION
    Script to Get list of Virtual Networks available in Azure Resource Manager Portal
    ...
    .EXAMPLE
    C:\PS> .\Get-ListOfVNets.ps1 -AzureUserName "testlab@netenrich.com" -AzurePassword 'pass12@w0rd' -AzureSubscriptionID ae7c7576-f01c-4026-9b94-d05e04e459fc 
...
Process
{
    ...
    Try
    {
        ($VNetList = Get-AzureRmResource -ErrorAction Stop -WarningAction SilentlyContinue | Where { $_.ResourceType -eq 'Microsoft.Network/virtualNetworks' } | Select ResourceName) | Out-Null
        $arrVNetList = $VNetList.ResourceName
        $VNetList = (@{ "Response" = [Array]$arrVNetList; "Status" = "Success"} | ConvertTo-Json)
        Write-Output $VNetList
    }
    Catch
    {
