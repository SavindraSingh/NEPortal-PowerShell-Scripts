<#
    ...
    .PARAMETER skuName
    Type of the Backup Vault that you are trying to create. The allowed values for this parameter are "Standard", "Premium"
    ...
    .NOTES
     Purpose of script: Create New backup vault in Azure Resource Manager Portal
     Minimum requirements: PowerShell Version 1.4.0
    ...
    .EXAMPLE
    C:\PS> .\Create-AzureRMBackupVaultFromTemplate.ps1 -ClientID 421 -AzureUserName 'testlab@netenrich.com' -AzurePassword 'pass12@word' -AzureSubscriptionID 'ae7c7576-f01c-4026-9b94-d05e04e459fc' -Location 'Central US' -ResourceGroupName 'TestLabRG' -DeploymentName 'SavindraTestFromPS' -TemplateJSONPath '.\JsonTemplates\BackupVaultTemplate.json' -ParameterJSONPath '.\JsonTemplates\BackupVaultParameters.json' -skuName 'Standard' -VaultName 'JsnTmplTestPSBVault' -WarningAction 'SilentlyContinue'
...
Begin
{
    ...
    Function Validate-AllParameters
    {
        Try
        {
            ...
            Else
            {
                $skuNameValues = @("Standard","Premium")
                If($skuName -in $skuNameValues)
                { <# Parameter value is valid #> }
                Else
                {
                    Write-LogFile -FilePath $LogFilePath -LogText "Validation failed. skuName '$skuName' is NOT a valid value for this parameter.`r`n<#BlobFileReadyForUpload#>"
                    $ObjOut = "Validation failed. skuName '$skuName' is not a valid value for this parameter."
                    $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                    Write-Output $output
                    Exit
                }
...
"@
        ...
        Write-LogFile -FilePath $LogFilePath -LogText "Backup vault '$VaultName' created successfully"
        $ObjOut = "Backup vault '$VaultName' created successfully"
        $output = (@{"Response" = [Array]$ObjOut; Status = "Success"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
        Write-Output $output
    }
    Catch
    {
        $ObjOut = "Error while creating backup vault '$VaultName'.`r`n$($Error[0].Exception.Message)"
        $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
        Write-Output $output
        Write-LogFile -FilePath $LogFilePath -LogText "$ObjOut`r`n<#BlobFileReadyForUpload#>"
        Exit
