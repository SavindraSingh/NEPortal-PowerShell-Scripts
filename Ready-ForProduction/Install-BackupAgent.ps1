<##
    ...
    .EXAMPLE
    C:\PS> .\Install-BackupAgent.ps1 clientID 12345 -AzureUserName Bhaskar@netenrich.com -AzurePassword Password1 -AzureSubscriptionID ca68598c-ecc3-4abc-b7a2-1ecef33f278d -ResourceGroupName testgrp -Location 'East Asia' -VmName testVM
    .LINK
...
Process
{
    ...
    # 6. Install the MARS Agent with the use of custom script
    Try
    {
        ...
        $ExtensionName = "InstallMARSAgent"
        Write-LogFile -FilePath $LogFilePath -LogText "Trying to Set the Extension for MARS Agent installation."
        ...
        if($MARSInstallExtensionStatus.StatusCode -eq 'OK')
        {
            ...
            if($InstallationStatus -ne $null)
            {
                ...
                if(($ExtScriptStatus.Statuses.Code -eq 'ProvisioningState/succeeded'))
                {
                    ...
                    {
                        Write-LogFile -FilePath $LogFilePath -LogText "Microsoft Azure Recovery Services Agent for backup has been installed successfully on $VMName.`r`n<#BlobFileReadyForUpload#>"
                        $ObjOut = "Microsoft Azure Recovery Services Agent for backup has been installed successfully on $VMName."
                        $output = (@{"Response" = [Array]$ObjOut; Status = "Success"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                        Write-Output $output
                    }
                    Else 
                    {
                        Write-LogFile -FilePath $LogFilePath -LogText "Microsoft Azure Recovery Services Agent for backup has not been installed successfully on $VMName.$message1.$message2`r`n<#BlobFileReadyForUpload#>"
                        $ObjOut = "Microsoft Azure Recovery Services Agent for backup has not been installed successfully on $VMName.$message1.$message2."
                        $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                        Write-Output $output
                        Exit                        
                    }
                ...
                {
                    Write-LogFile -FilePath $LogFilePath -LogText "Provisioning the script for MARS Agent installation was failed on $VMName.`r`n<#BlobFileReadyForUpload#>"
                    $ObjOut = "Provisioning the script for MARS Agent installation was failed on $VMName."
                    $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                    Write-Output $output
                    Exit
                }
            ...
            {
                Write-LogFile -FilePath $LogFilePath -LogText "The extension was not installed for installing MARS Agent on VM $VMName.`r`n<#BlobFileReadyForUpload#>"
                $ObjOut = "The extension was not installed for installing MARS Agent on VM $VMName."
                $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                Write-Output $output
                Exit
            }           
        ...
        Else
        {
            Write-LogFile -FilePath $LogFilePath -LogText "Unable to install the custom script extension installing MARS Agent on Virtual Machine $VMName.`r`n<#BlobFileReadyForUpload#>"
            $ObjOut = "Unable to install the custom script extension installing MARS Agent on Virtual Machine $VMName."
            $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
            Write-Output $output
            Exit            
        }
    ...
    {
        $ObjOut = "Error while setting the script for Installing the MARS Agent on $VMName virtual Machine.$($Error[0].Exception.Message)"
        $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
        Write-Output $output
        Write-LogFile -FilePath $LogFilePath -LogText "$ObjOut`r`n<#BlobFileReadyForUpload#>"
        Exit
