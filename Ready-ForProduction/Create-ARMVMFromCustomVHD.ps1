<#
    .SYNOPSIS
    Script to create New Virual Machine in Azure Resource Manager Portal from VHD

    .DESCRIPTION
    Script to create New Virtual Machine in Azure Resource Manager Portal from VHD
    ...
    Azure VM Custom Image blob url.
    ...
    .NOTES
     Purpose of script: The script is to create VM from custom VHD
    ...
    Function Validate-AllParameters
    {
        Try
        {
            ...
            If([String]::IsNullOrEmpty($OSDiskUri))
            {
                ...
                $ObjOut = "Validation failed. Location OSDiskUri value is empty."
                $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                Write-Output $output
                Exit
            }

            # Validate parameter: OSType
            Write-LogFile -FilePath $LogFilePath -LogText "Validating Parameters: OSType. Only ERRORs will be logged."
            If([String]::IsNullOrEmpty($OSType))
            {
                Write-LogFile -FilePath $LogFilePath -LogText "Validation failed. OSType parameter value is empty.`r`n<#BlobFileReadyForUpload#>"
                $ObjOut = "Validation failed. Location OSType value is empty."
                $output = (@{"Response" = [Array]$ObjOut; Status = "Failed"; BlobURI = $LogFileBlobURI} | ConvertTo-Json).ToString().Replace('\u0027',"'")
                Write-Output $output
                Exit
            }
            ...
            Else
            {
                ...
                Else
                {
                    Write-LogFile -FilePath $LogFilePath -LogText "File/path not found '$TemplateJSONPath'."
...
Process
{
    ...
    #5. Checking whether the given location suports the VM Size
    try
    {
    ...
    # 6. Checking for the Virtual Network and Subnet existence
    Try
    {
    ...
    # 7. Checking for the Storage Account existence
    Try
    {
    ...
    # 8. Checking for the Public IP Existence
    Try
    {
    ...
    # 9. Create Virtual Machine using Image from Gallary
    try
    {
        ...
        $ParameterJSONText = @"
{
    ...
    "parameters": {
                        ...
                        "availabiltySet":{"value":"$AvailabilitySetName"}
                  }
}
"@
    ($ParameterJSONText | Out-File -FilePath $ParameterJSONPath -ErrorAction Stop) | Out-Null
        ...
        if($Status.ProvisioningState -eq 'Succeeded')
        {
            ...
            $output = (@{"Response" = [Array]$ObjOut; Status = "Success"; BlobURI = $LogFileBlobUR} | ConvertTo-Json).ToString().Replace('\u0027',"'")
            Write-Output $output
        }
        else
        {
